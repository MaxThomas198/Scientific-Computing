...
class drv(object):
    ...
    def drv_prod(expr):
        m1 = expr.get_mult1()
        m2 = expr.get_mult2()
        if isinstance(m1, const) and isinstance(m2, const):
            return const.mult(m1, m2)
        else:
            return maker.make_prod(m1, drv.drv_pwr(m2))
    ...
    def drv_plus(expr):
        if isinstance(expr.get_elt2(), const):
            ...
            return maker.make_plus(drv.drv(expr.get_elt1()), drv.drv(expr.get_elt2()))
